...
import argparse
import numpy as np
from model.model2 import NEGAT
...
parser.add_argument('--early_stop_maxtry', type=int, default=10)
...
def main():
    ...
    # lr_scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda epoch : np.sin(epoch) / epoch)
    ...
    for i in range(args.epoch):
        ...
        if i < args.warmup_step:
            curr_lr = args.lr * i / args.warmup_step
...
if __name__ == "__main__":
    ...
    # a[0][0] = 1
    # print(a)
    # a = torch.where(((a <= 0.9) & (a >= 0.5)) | (a == 1), a, torch.zeros_like(a))
    # print(a)