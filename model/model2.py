...
class NEGATLayer(nn.Module):
    def __init__(self, cuda, adj, num_nodes, num_features, num_timesteps_input,
        ...
        super(NEGATLayer, self).__init__()
    ...
    def calLoss(self, x, A, a=1, b=0.01):
        loss = 0
        if self.training:
            n = A.shape[1]
...
class NEGAT(nn.Module):
    def __init__(self, cuda, adj, num_nodes, num_features, num_timesteps_input,
        ...
        self.nets = [NEGATLayer(cuda, adj, num_nodes, num_features, num_timesteps_input, num_timesteps_output) for _ in range(2)]
        self.nets = nn.ModuleList(self.nets)
        self.end_conv = EndConv(nhid * 2, 12, 512)

    def forward(self, X, A_hat, SE, a=0.5):
        norm_loss = 0
        if self.training:
            out1, loss1 = self.nets[0](X, A_hat, SE, a)
            norm_loss += loss1
            out2, loss2 = self.nets[1](X, A_hat, SE, a)
            norm_loss += loss2
            ...
            out1 = self.nets[0](X, A_hat, SE, a)
            out2 = self.nets[1](X, A_hat, SE, a)
            out = torch.cat([out1, out2], dim=2)
            out = self.end_conv(out)
            out = out.reshape(out.shape[0], out.shape[1], out.shape[2], 1)
            return out
